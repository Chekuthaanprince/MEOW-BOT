...
const events = require("./events");/* Copyright (C) 2020 Yusuf Usta.
RECODDED BY AFNANPPLK
Plk  
*/
...
const {Message, StringSession, Image, Video} = require('./MEOW-BOTMwol/');
       ...
       if (msg.messageStubType === 32 || msg.messageStubType === 28) {
	    ...
	    var afn_plk_ = '```⏱ Time :' + plk_say + '```\n```📅 Date :' + plk_here + '```'
            ...
            if (gb !== false) {
                ...
                //created by afnanplk
                    var plkMEOW-BOT = await axios.get(config.GIF_BYE, { responseType: 'arraybuffer' })
                    var pinkjson = await conn.groupMetadata(msg.key.remoteJid)
                await conn.sendMessage(msg.key.remoteJid, Buffer.from(plkMEOW-BOT.data), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message.replace('{gif}', '').replace('{time}', afn_plk_).replace('{gphead}', pinkjson.subject).replace('{gpmaker}', pinkjson.owner).replace('{gpdesc}', pinkjson.desc).replace('{owner}', conn.user.name) });
            } else {
...
//====== m & c =====	
	       ...
	       var afn_plk_ = '```⏱ Time :' + plk_say + '```\n```📅 Date :' + plk_here + '```'
            ...
            if (gb !== false) {		    
                ...
                var plkMEOW-BOT = await axios.get(config.WEL_GIF, { responseType: 'arraybuffer' })
                var pinkjson = await conn.groupMetadata(msg.key.remoteJid)
                await conn.sendMessage(msg.key.remoteJid, Buffer.from(plkMEOW-BOT.data), MessageType.video, {mimetype: Mimetype.gif, caption: gb.message.replace('{gif}', '').replace('{time}', afn_plk_).replace('{gphead}', pinkjson.subject).replace('{gpmaker}', pinkjson.owner).replace('{gpdesc}', pinkjson.desc).replace('{owner}', conn.user.name).replace('{no fake}', '').replace('{mention}', poison ).replace('{count}', count),contextInfo: {mentionedJid: [user]} });
            } else {
                var pinkjson = await conn.groupMetadata(msg.key.remoteJid)
 ...
 //========================================end of reminder===========================================	    
		...
		try {
		            ...
		            if (error.message.includes('includes')) return;
                            if (config.LANG == 'ES') {
                                await conn.sendMessage(conn.user.jid, '*🤖  ERROR ANALYSIS [MEOW-BOT] *' + 
                                    '\n*MEOW-BOT bot ha tenido un error*'+
                                ...
                                await conn.sendMessage(conn.user.jid, '*🤖 ERROR ANALYSIS [MEOW-BOT] *' + 
                                    '\n*MEOW-BOT Bot an error has occurred!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*🤖 ERROR ANALYSIS [MEOW-BOT] *' + 
                                        '\n========== ```Error Resolved!``` ==========' +
